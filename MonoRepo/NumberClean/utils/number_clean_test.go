...
func BenchmarkWordToNum(t *testing.B) {
	tests := []testWordToNum{
		...
		{
			name:           "one one nine point five three",
			s:              "erebus for thirty fly heading two o two plain to flight level one ninety seven speed one hundred twenty six knots",
			expectedResult: "erebus for 30 fly heading 202 plain to flight level 197 speed 126 knots",
		},
	}
